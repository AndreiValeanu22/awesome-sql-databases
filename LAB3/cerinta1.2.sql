...
SET VERIFY OFF -- ca sa nu mai afiseze fiecare inlocuire de variabila, pentru ca o sa fie enervant daca avem de inlocuit de multe ori. NU inteleg.

/*
DE CE SE PUNE SET VERIFY OFF? Pentru ca atunci cand folosim variabilele de substitutie (&), SQL*Plus afiseaza comanda cu valorile inlocuite, 
ceea ce poate fi enervant daca avem multe astfel de variabile. Prin setarea VERIFY OFF, dezactivam acest comportament, astfel incat SQL*Plus nu va mai afisa 
comanda cu valorile inlocuite, ci doar va executa comanda cu valorile introduse de utilizator.
*/

SELECT nume,
       id_departament,
       12 * (salariu + NVL(comision,0)) AS venit_anual -- e important sa pui si NVL? Da, pentru ca daca comisionul este NULL, atunci NVL va inlocui valoarea NULL cu 0, astfel incat calculul venitului anual sa fie corect. Daca nu am pune NVL, atunci orice angajat care are comision NULL ar avea un venit anual calculat ca NULL, ceea ce nu este corect.
FROM angajati
WHERE id_departament = &&dep_id
  AND 12 * (salariu + NVL(comision,0)) > &&venit_med_annual;

/*
Care e diferenta dintre & si && si de ce s-au inventat ambele?

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

Diferența e **persistența valorii**:

- `&var` → **îți cere valoarea de fiecare dată** când apare în comandă.  
- `&&var` → **îți cere o singură dată**, apoi **o reține** și o refolosește în sesiunea curentă.

De ce există ambele:
- `&` e util când vrei **valori diferite** la fiecare execuție sau chiar de mai multe ori în același statement.
- `&&` e util când vrei **aceeași valoare** reutilizată în mai multe comenzi sau de mai multe ori în același script.

Poți șterge o variabilă reținută:
```
UNDEFINE var
```
*/
